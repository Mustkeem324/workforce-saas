...
import { IndianRupee, Download, ChevronLeft, ChevronRight, FileText, Sparkles, TrendingUp } from 'lucide-react';
...
const PAYSLIP_CAROUSEL: MonthlyPayslipCard[] = [
  { id: 'ps-aug', month: 'August 2026', payPeriod: 'Jul 16 - Jul 31, 2026', grossPay: 34000.00, netPay: 28500.00, deductions: 5500.00, status: 'PAID' },
  { id: 'ps-jul', month: 'July 2026', payPeriod: 'Jul 01 - Jul 15, 2026', grossPay: 34000.00, netPay: 28500.00, deductions: 5500.00, status: 'PAID' },
  { id: 'ps-jun', month: 'June 2026', payPeriod: 'Jun 16 - Jun 30, 2026', grossPay: 32000.00, netPay: 26800.00, deductions: 5200.00, status: 'PAID' }
];
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-[var(--bg-surface-raised)] border border-[var(--border-default)] p-5 rounded-2xl shadow-[var(--shadow-1)]">
        <div>
          <div className="flex items-center gap-2">
            <IndianRupee className="w-5 h-5 text-[var(--accent-500)]" />
      ...
      <Card elevation={1} className="p-6 bg-gradient-to-r from-[var(--accent-500)]/15 via-[var(--bg-surface-raised)] to-[var(--bg-surface-raised)] border border-[var(--accent-500)]/30 flex flex-wrap items-center justify-between gap-4 font-mono tabular-nums">
        <div>
          ...
          <div className="text-3xl font-black text-[var(--text-primary)]">₹4,85,000.00</div>
        </div>
        <Badge variant="success">+₹42,000 vs 2025</Badge>
      </Card>
        ...
        <Card elevation={2} className="space-y-4 p-6 border-2 border-[var(--border-accent)]/40 relative">
          ...
          <div className="space-y-3 font-mono tabular-nums text-xs">
            <div className="flex justify-between items-center p-3 rounded-xl bg-[var(--bg-canvas)]">
              ...
              <span className="font-bold text-emerald-400 text-sm">₹{currentPayslip.grossPay.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>

            <div className="flex justify-between items-center p-3 rounded-xl bg-[var(--bg-canvas)]">
              <span className="font-sans text-[var(--text-secondary)] font-semibold">Total Tax & Benefit Deductions:</span>
              <span className="font-bold text-rose-400 text-sm">-₹{currentPayslip.deductions.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            ...
            <div className="flex justify-between items-center p-3 rounded-xl bg-[var(--accent-500)]/15 border border-[var(--accent-500)]/40 font-bold">
              ...
              <span className="text-[var(--accent-500)] text-base">₹{currentPayslip.netPay.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
